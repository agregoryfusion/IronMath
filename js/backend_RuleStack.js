// backend_RuleStack.js - Supabase + leaderboard + caching for the Rule Stack game
...
const TABLES = {
  leaderboard: "rulestack_leaderboard",
  sessions: "rulestack_sessions",
  questions: "rulestack_questions"
};
...
function normalizeLeaderboardRow(r) {
  ...
  return {
    ...
    maxRules: r.max_rules_used ?? null,
    hardestDifficulty: r.hardest_rule_difficulty ?? null,
    finalTotal: r.final_time_seconds ?? r.total_time_seconds ?? null
  };
}
...
function renderLeaderboard(rows) {
  ...
  rows.forEach((row, idx) => {
    ...
    const penalty = Number(row.penaltyTime ?? 0);
    const rawTime = Math.max(0, time - penalty);
    const q = row.questionsAnswered || 0;
    const tpq = q > 0 ? rawTime / q : 0;
    tr.appendChild(t(idx + 1));
    tr.appendChild(t(row.playerName || "?"));
    tr.appendChild(t(q));
    tr.appendChild(t(row.maxRules ?? "—"));
    tr.appendChild(t(rawTime.toFixed(2)));
    tr.appendChild(t(tpq.toFixed(2)));
    tr.appendChild(t((row.finalTotal ?? row.totalTime ?? 0).toFixed(2)));
