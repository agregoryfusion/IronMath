// backend_WorldCapitals.js - Supabase helpers for World Capitals game
...
const TABLES = {
  leaderboard: "worldcapitals_leaderboard",
  sessions: "worldcapitals_sessions",
  questions: "worldcapitals_questions"
};
...
function normalizeLeaderboardRow(r) {
  ...
  return {
    ...
    countriesCorrect: r.countries_correct ?? 0,
...
function renderLeaderboard(rows) {
  ...
  rows.forEach((row, idx) => {
    ...
    tr.appendChild(t(row.countriesCorrect ?? 0));
...
async function loadLeaderboard(scopeFilter = "all", timeFilter = "monthly", forceRefresh = false) {
  ...
  let query = supabase
    ...
    .select("*")
    .order("countries_correct", { ascending: false })
  ...
  const best = Object.values(grouped).map((list) =>
    list.sort((a, b) => {
      if (b.countriesCorrect === a.countriesCorrect) return a.totalTime - b.totalTime;
      return b.countriesCorrect - a.countriesCorrect;
    })[0]
  );
