// backend_ElementQuiz.js - Supabase helpers for Periodic Sprint game
...
const TABLES = {
  leaderboard: "elementquiz_leaderboard",
  sessions: "elementquiz_sessions",
  questions: "elementquiz_questions"
};
...
function normalizeLeaderboardRow(r) {
  if (!r) return null;
  const symbols = Number(r.symbols_correct ?? r.abbreviations_correct ?? 0);
  const numbers = Number(r.atomic_numbers_correct ?? 0);
  return {
    playerName: r.player_name,
    symbolsCorrect: symbols,
    numbersCorrect: numbers,
    totalPoints: symbols + numbers,
...
function renderLeaderboard(rows) {
  ...
  rows.forEach((row, idx) => {
    ...
    tr.appendChild(t(row.symbolsCorrect ?? 0));
    tr.appendChild(t(row.numbersCorrect ?? 0));
    tr.appendChild(t(row.totalPoints ?? 0));
...
async function loadLeaderboard(scopeFilter = "all", timeFilter = "monthly", forceRefresh = false) {
  ...
  let query = supabase
    ...
    .select("*")
    .order("symbols_correct", { ascending: false })
    .order("atomic_numbers_correct", { ascending: false })
  ...
  const best = Object.values(grouped).map((list) =>
    list.sort((a, b) => {
      if (b.totalPoints === a.totalPoints) return a.totalTime - b.totalTime;
